-- =====================================================================
--  45 — OYLIK TURI HAR XODIMGA ALOHIDA
--
--  MUAMMO. "Asosiy haq qanday hisoblanadi" sozlamasi butun maktabga
--  BITTA edi: yo hammasi qat'iy oylik, yo hammasi soatbay.
--
--  Haqiqiy maktabda bunday emas. Bir vaqtning o'zida:
--
--    · sinf rahbari — qat'iy oylik oladi;
--    · fan o'qituvchisi — stavka bo'yicha;
--    · to'garak rahbari yoki chaqiriladigan mutaxassis — soatbay.
--
--  Ilgari direktor bittasini tanlashi kerak edi va qolganlarining
--  oyligi noto'g'ri chiqardi. Buni tuzatishning yagona yo'li — har
--  oy qo'lda tuzatuv kiritish, ya'ni tizimdan foydalanmaslik.
--
--  YECHIM. O'qituvchi kartochkasida tur tanlanadi. Tanlanmagan bo'lsa
--  maktab sozlamasi ishlaydi — ya'ni mavjud maktablarda hech narsa
--  o'zgarmaydi va hamma xodimni qayta sozlash shart emas.
-- =====================================================================
...
begin
  if not exists (
    select 1 from pg_constraint
     where conname = 'teachers_base_type_check'
       and conrelid = 'public.teachers'::regclass
  ) then
    alter table public.teachers
      add constraint teachers_base_type_check
      check (base_type is null
             or base_type in ('fixed', 'rate', 'hourly', 'mixed'));
  end if;
end $do$;

comment on column public.teachers.base_type is
  'Shu xodim uchun oylik turi. Bo''sh bo''lsa maktabning umumiy '
  'sozlamasi (payroll_settings.base_type) ishlatiladi.';
...
comment on function public.calc_payroll(uuid, date) is
  'TZ 4.11 — bir o''qituvchining bir oylik hisobi. Oylik turi avval '
  'xodim kartochkasidan, u bo''sh bo''lsa maktab sozlamasidan olinadi. '
